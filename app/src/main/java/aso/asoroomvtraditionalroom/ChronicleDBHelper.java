...
public class ChronicleDBHelper extends SQLiteOpenHelper {
    ...
    public void logAllRuns() {
        Cursor csr = db.query(
                CHRONICLE_TABLENAME,
                new String[]{CHRONICLE_COL_RUNNUMBER},
                null, null,
                CHRONICLE_COL_RUNNUMBER,null,
                CHRONICLE_COL_RUNNUMBER + " ASC");
        while (csr.moveToNext()) {
    ...
    public void logAllRunStats() {
        ...
        /*
            WITH c1 AS(SELECT max(timestamp) - min(timestamp) AS sum, run_number, datetime(min(timestamp)/1000,'unixepoch') AS started, datetime(max(timestamp)/1000,'unixepoch') AS finished
            FROM Chronicle as c1
            GROUP BY run_number ORDER BY timestamp)
